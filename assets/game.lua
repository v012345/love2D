...
function Game:start_up()
    ...
    if settings then
        ...
        if G.VERSION >= '1.0.0' and (love.system.getOS() == 'NOPE')
            and ((not settings_file.version) or (settings_file.version < '1.0.0')) then
            for i = 1, 3 do
    ...
    self.shared_undiscovered_joker = Sprite(
        0, 0, self.CARD_W, self.CARD_H, self.ASSET_ATLAS["centers"], self.P_CENTERS.undiscovered_joker.pos
    )
    self.shared_undiscovered_tarot = Sprite(
        0, 0, self.CARD_W, self.CARD_H, self.ASSET_ATLAS["centers"], self.P_CENTERS.undiscovered_tarot.pos
    )

    self.shared_sticker_eternal = Sprite(0, 0, self.CARD_W, self.CARD_H, self.ASSET_ATLAS["stickers"], { x = 0, y = 0 })
    self.shared_sticker_perishable = Sprite(
        0, 0, self.CARD_W, self.CARD_H, self.ASSET_ATLAS["stickers"],
        {
            x = 0,
            y = 2
        }
...
function Game:init_item_prototypes()
    ...
    if not love.filesystem.getInfo(G.SETTINGS.profile .. '/' .. 'meta.jkr') then
        love.filesystem.append(G.SETTINGS.profile .. '/'
                .. 'meta.jkr', 'return {}')
    end
    ...
    for k, v in pairs(self.P_CENTERS) do
        if not v.wip and not v.demo then
            ...
            if not v.unlocked and (string.find(k, '^j_') or string.find(k, '^b_') or string.find(k, '^v_'))
                and meta.unlocked[k] then
                v.unlocked = true
            end
            if not v.unlocked and (string.find(k, '^j_') or string.find(k, '^b_') or string.find(k, '^v_')) then
                self.P_LOCKED[#self.P_LOCKED + 1] = v
            end
            if not v.discovered and (string.find(k, '^j_') or string.find(k, '^b_')
                    or string.find(k, '^e_') or string.find(k, '^c_')
                    or string.find(k, '^p_') or string.find(k, '^v_')) and meta.discovered[k] then
                v.discovered = true
            end
    ...
    if not self.LANGUAGES then
        ...
        self.LANGUAGES = {
            ...
            ['de'] = {
                ...
                warning = {
                    'Diese Übersetzung ist noch im Beta-Stadium. Willst du uns helfen,',
                    'sie zu verbessern? Dann klicke bitte auf die Feedback-Taste.', "Zum Bestätigen erneut klicken"
                }
            },
            ['es_419'] = {
                font = 1,
                ...
                warning = {
                    'Este idioma todavía está en Beta. Pulsa el botón', 'de sugerencias para ayudarnos a mejorarlo.',
            ...
            ['fr'] = {
                ...
                warning = {
                    'La traduction française est encore en version bêta. ',
                    'Veuillez cliquer sur le bouton pour nous donner votre avis.', "Cliquez à nouveau pour confirmer"
                }
            },
            ['id'] = {
                font = 1,
                ...
                warning = {
                    'Bahasa ini masih dalam tahap Beta. Untuk membantu',
            ...
            ['it'] = {
                ...
                warning = {
                    'Questa traduzione è ancora in Beta. Per', 'aiutarci a migliorarla, clicca il tasto feedback',
            ...
            ['nl'] = {
                ...
                warning = {
                    'Deze taal is nog in de Beta fase. Help ons het te ',
            ...
            ['pl'] = {
                ...
                warning = {
                    'Polska wersja językowa jest w fazie Beta. By pomóc nam poprawić',
            ...
            ['pt_BR'] = {
                ...
                warning = {
                    'Esta tradução ainda está em Beta. Se quiser nos ajudar',
            ...
            ['ru'] = {
                ...
                warning = {
                    'Этот язык все еще находится в Бета-версии. Чтобы помочь',
...
function Game:set_render_settings()
    ...
    self.animation_atli = {
        {
            name = "blind_chips",
            path = "resources/textures/" .. self.SETTINGS.GRAPHICS.texture_scaling
                .. "x/BlindChips.png",
            px = 34,
            py = 34,
            frames = 21
    ...
    for i = 1, #self.animation_atli do
        ...
        self.ANIMATION_ATLAS[self.animation_atli[i].name].image = love.graphics.newImage(
            self.animation_atli[i].path,
            {
                mipmaps = true,
                dpiscale = self
                    .SETTINGS
    ...
    for i = 1, #self.asset_atli do
        ...
        self.ASSET_ATLAS[self.asset_atli[i].name].image = love.graphics.newImage(
            self.asset_atli[i].path,
            {
                mipmaps = true,
                dpiscale = self
                    .SETTINGS
    ...
    for i = 1, #self.asset_images do
        ...
        self.ASSET_ATLAS[self.asset_images[i].name].image = love.graphics.newImage(
            self.asset_images[i].path,
            {
                mipmaps = true,
                dpiscale = 1
            }
...
function Game:sandbox()
    ...
    G.SPLASH_BACK = Sprite(
        -30, -13, G.ROOM.T.w + 60, G.ROOM.T.h + 22, G.ASSET_ATLAS["ui_" .. (G.SETTINGS.colourblind_option and 2 or 1)],
        {
            x = 2,
            y = 0
        }
...
function Game:splash_screen()
    ...
    G.E_MANAGER:add_event(Event({
            ...
            func = (function ()
                ...
                G.SPLASH_BACK = Sprite(
                    -30, -13, G.ROOM.T.w + 60, G.ROOM.T.h + 22, G.ASSET_ATLAS["ui_1"], { x = 2, y = 0 }
                )
                G.SPLASH_BACK:define_draw_steps({
                    {
                        shader = 'splash',
                ...
                G.E_MANAGER:add_event(Event({
                        ...
                        func = (function ()
                            local SC_scale = 1.2
                            SC = Card(
                                G.ROOM.T.w / 2 - SC_scale * G.CARD_W / 2, 10. + G.ROOM.T.h / 2 - SC_scale * G.CARD_H / 2,
                ...
                function make_splash_card(args)
                    ...
                    local card = Card(
                        ...
                        card_pos.y + G.ROOM.T.h / 2 - G.CARD_H * card_size / 2, card_size * G.CARD_W,
                        card_size * G.CARD_H, pseudorandom_element(G.P_CARDS), G.P_CENTERS.c_base
                    )
                                ...
                                G.E_MANAGER:add_event(Event({
                                        ...
                                        func = (function ()
                                            if card.T.scale <= 0 then
                                                if temp_i < 30 then
                                                    play_sound(
                                                        'whoosh1', temp_pitch + math.random() * 0.05,
                                                        0.25 * (1 - temp_i / 50)
                                                    )
                                                end
...
function Game:main_menu(change_context) -- True if main menu is accessed from the splash screen, false if it is skipped or accessed from the game
    ...
    local CAI = { TITLE_TOP_W = G.CARD_W, TITLE_TOP_H = G.CARD_H }
    self.title_top = CardArea(0, 0, CAI.TITLE_TOP_W, CAI.TITLE_TOP_H, { card_limit = 1, type = 'title' })

    G.SPLASH_LOGO = Sprite(
        0, 0, 13 * SC_scale, 13 * SC_scale * (G.ASSET_ATLAS["balatro"].py / G.ASSET_ATLAS["balatro"].px),
    ...
    local replace_card = Card(
        self.title_top.T.x, self.title_top.T.y, 1.2 * G.CARD_W * SC_scale, 1.2 * G.CARD_H * SC_scale, G.P_CARDS.S_A,
        G.P_CENTERS.c_base
    )
    ...
    G.E_MANAGER:add_event(Event({
            ...
            func = (function ()
                play_sound(
                    'magic_crumple' .. (change_context == 'splash' and 2 or 3),
    ...
    if replace_card and (G.P_CENTERS.j_blueprint.unlocked) then
        ...
        if #viable_unlockables > 0 then
            ...
            G.E_MANAGER:add_event(Event({
                    ...
                    func = (function ()
                        card = Card(
                            self.title_top.T.x, self.title_top.T.y, 1.2 * G.CARD_W * SC_scale, 1.2 * G.CARD_H * SC_scale,
                            nil, pseudorandom_element(viable_unlockables) or self.P_CENTERS.j_joker
                        )
    ...
    UIBox {
        definition = {
            n = G.UIT.ROOT,
            config = { align = "cm", colour = G.C.UI.TRANSPARENT_DARK },
            nodes = {
...
function Game:start_run(args)
    ...
    if not saveTable then
        ...
        ease_background_colour_blind(
            G.STATE, saveTable.BLIND.name:gsub("%s+", "") ~= '' and saveTable.BLIND.name or 'Small Blind'
        )
    end

    local selected_back = saveTable and saveTable.BACK.name or (args.challenge and args.challenge.deck
            and args.challenge.deck.type) or (self.GAME.viewed_back and self.GAME.viewed_back.name)
        or self.GAME.selected_back and self.GAME.selected_back.name or 'Red Deck'
    ...
    G.C.UI_CHIPS[1], G.C.UI_CHIPS[2], G.C.UI_CHIPS[3], G.C.UI_CHIPS[4] = G.C.BLUE[1], G.C.BLUE[2], G.C.BLUE[3], G
            .C
            .BLUE
            [4]
    ...
    if not saveTable then
        ...
        self.GAME.pseudorandom.seed = args.seed or (not (G.SETTINGS.tutorial_complete
                or G.SETTINGS.tutorial_progress.completed_parts['big_blind'])
                and "TUTORIAL") or generate_starting_seed()
    end

    for k, v in pairs(self.GAME.pseudorandom) do
        if v == 0 then self.GAME.pseudorandom[k] = pseudohash(k..self.GAME.pseudorandom.seed) end
    end
    ...
    if not saveTable then
        ...
        self.GAME.current_round.voucher = G.SETTINGS.tutorial_progress and G.SETTINGS.tutorial_progress.forced_voucher
            or get_next_voucher_key()
        self.GAME.round_resets.blind_tags.Small = G.SETTINGS.tutorial_progress and G.SETTINGS.tutorial_progress.forced_tags
            and G.SETTINGS.tutorial_progress.forced_tags[1]
            or get_next_tag_key()
        self.GAME.round_resets.blind_tags.Big = G.SETTINGS.tutorial_progress and G.SETTINGS.tutorial_progress.forced_tags
            and G.SETTINGS.tutorial_progress.forced_tags[2]
            or get_next_tag_key()
    else
    ...
    self.consumeables = CardArea(
        0, 0, CAI.consumeable_W, CAI.consumeable_H,
        { card_limit = self.GAME.starting_params.consumable_slots, type = 'joker', highlight_limit = 1 }
    )

    self.jokers = CardArea(
        0, 0, CAI.joker_W, CAI.joker_H,
        { card_limit = self.GAME.starting_params.joker_slots, type = 'joker', highlight_limit = 1 }
    )

    self.discard = CardArea(0, 0, CAI.discard_W, CAI.discard_H, { card_limit = 500, type = 'discard' })
    self.deck = CardArea(0, 0, CAI.deck_W, CAI.deck_H, { card_limit = 52, type = 'deck' })
    self.hand = CardArea(
        0, 0, CAI.hand_W, CAI.hand_H, { card_limit = self.GAME.starting_params.hand_size, type = 'hand' }
    )
    self.play = CardArea(0, 0, CAI.play_W, CAI.play_H, { card_limit = 5, type = 'play' })
    ...
    if saveTable then
        ...
        if not card_protos then
            ...
            for k, v in pairs(self.P_CARDS) do
                ...
                if keep then
                    card_protos[#card_protos + 1] = { s = _s, r = _r, e = _e, d = _d, g = _g }
                end
            end
        end
...
function Game:update(dt)
    ...
    if not G.fbf or G.new_frame then
        ...
        self.SPEEDFACTOR = (G.STAGE == G.STAGES.RUN and not G.SETTINGS.paused and not G.screenwipe)
            and self.SETTINGS.GAMESPEED
            or 1
    ...
    if G.prev_small_state ~= G.GAME.round_resets.blind_states.Small or G.prev_large_state
        ~= G.GAME.round_resets.blind_states.Big
        or G.prev_boss_state ~= G.GAME.round_resets.blind_states.Boss or G.RESET_BLIND_STATES then
    ...
    if G.STEAM and G.STEAM.send_control.update_queued
        and (G.STEAM.send_control.force or G.STEAM.send_control.last_sent_stage ~= G.STAGE
            or G.STEAM.send_control.last_sent_time < G.TIMERS.UPTIME - 120) then
    ...
    if G.FILE_HANDLER and G.FILE_HANDLER and G.FILE_HANDLER.update_queued
        and (G.FILE_HANDLER.force or G.FILE_HANDLER.last_sent_stage ~= G.STAGE
            or ((G.FILE_HANDLER.last_sent_pause ~= G.SETTINGS.paused) and G.FILE_HANDLER.run)
            or (not G.FILE_HANDLER.last_sent_time
                or (G.FILE_HANDLER.last_sent_time < (G.TIMERS.UPTIME - G.F_SAVE_TIMER)))) then
...
function Game:draw()
    ...
    if not G.debug_UI_toggle then
        ...
        if G.debug_splash_size_toggle then
            ...
            if (not self.OVERLAY_MENU) or (not self.F_HIDE_BG) then
                ...
                for k, v in pairs(self.I.UIBOX) do
                    if not v.attention_text and not v.parent
                        and v ~= self.OVERLAY_MENU and v ~= self.screenwipe
                        and v ~= self.OVERLAY_TUTORIAL and v ~= self.debug_tools
                        and v ~= self.online_leaderboard and v ~= self.achievement_notification then
                        love.graphics.push()
                        v:translate_container()
                        v:draw()
                        love.graphics.pop()
            ...
            if self.CONTROLLER.focused.target and getmetatable(self.CONTROLLER.focused.target) == Card
                and (self.CONTROLLER.focused.target.area ~= G.hand
                    or self.CONTROLLER.focused.target == self.CONTROLLER.dragging.target) then
                love.graphics.push()
            ...
            love.graphics.translate(
                -self.CURSOR.T.w * G.TILESCALE * G.TILESIZE * 0.5,
                -self.CURSOR.T.h * G.TILESCALE
                    * G.TILESIZE * 0.5
            )
            self.CURSOR:draw()
            love.graphics.pop()
    ...
    do
        ...
        if (not G.recording_mode or G.video_control) and true then
            G.ARGS.eased_cursor_pos = G.ARGS.eased_cursor_pos
                or {
                    x = G.CURSOR.T.x,
                    y = G.CURSOR.T.y,
                    sx = G
            ...
            G.screenwipe_amt = G.screenwipe_amt
                and (0.95 * G.screenwipe_amt + 0.05 * ((self.screenwipe and 0.4 or self.screenglitch and 0.4) or 0))
                or 1
            ...
            G.SHADERS['CRT']:send(
                'mouse_screen_pos',
                G.video_control and { love.graphics.getWidth() / 2, love.graphics.getHeight() / 2 }
                    or {
                        G
                            .ARGS
                            .eased_cursor_pos
        ...
        if G.check and G.SETTINGS.perf_mode then
            ...
            for a, b in ipairs({ G.check.update, G.check.draw }) do
                for k, v in ipairs(b.checkpoint_list) do
                    ...
                    love.graphics.print(
                        v.label .. ': ' .. (string.format("%.2f", 1000 * (v.average or 0))) .. '\n', 10,
                        -section_h + 30
                            + v_off
                    )
...
function Game:state_col(_state)
    return (_state * 15251252.2 / 5.132) % 1,
        (_state * 1422.5641311 / 5.42) % 1, (_state * 1522.1523122 / 5.132) % 1,
        1
end

function Game:update_selecting_hand(dt)
    if not self.deck_preview and not G.OVERLAY_MENU
        and ((self.deck and self.deck.cards[1] and self.deck.cards[1].states.collide.is and ((not self
                .deck
                .cards
                [1]
                .states
                .drag
                .is)
                or self.CONTROLLER.HID.touch) and (not self
                .CONTROLLER
                .HID
                .controller)) or G.CONTROLLER.held_buttons.triggerleft) then
        if self.buttons then
        ...
        self.E_MANAGER:add_event(Event({
                ...
                blockable = false,
                func = function ()
                    if self.deck_preview
                        and not (((self.deck and self.deck.cards[1]
                            and self.deck.cards[1].states.collide.is and not self
                                .CONTROLLER
                                .HID
                                .controller))
                            or G.CONTROLLER.held_buttons.triggerleft) then
                        self.deck_preview:remove()
                        self.deck_preview = nil
                                ...
                                if math.abs(G.shop.T.y - G.shop.VT.y) < 3 then
                                    ...
                                    if not shop_exists then
                                        ...
                                        if G.load_shop_vouchers then
                                            ...
                                            if G.GAME.current_round.voucher
                                                and G.P_CENTERS[G.GAME.current_round.voucher] then
                                                local card = Card(
                                                    G.shop_vouchers.T.x + G.shop_vouchers.T.w / 2, G.shop_vouchers.T.y,
                                        ...
                                        if G.load_shop_booster then
                                            ...
                                            for i = 1, 2 do
                                                ...
                                                if G.GAME.current_round.used_packs[i] ~= 'USED' then
                                                    local card = Card(
                                                        G.shop_booster.T.x + G.shop_booster.T.w / 2, G.shop_booster.T.y,
    ...
    if not G.STATE_COMPLETE then
        ...
        G.E_MANAGER:add_event(Event({
                ...
                func = function ()
                    ...
                    if G.GAME.current_round.hands_played == 0 and G.GAME.current_round.discards_used == 0
                        and G.GAME.facing_blind then
        ...
        G.E_MANAGER:add_event(Event({
                ...
                func = function ()
                    G.booster_pack_sparkles = Particles(1, 1, 0, 0, {
                        ...
                        colours = {
                            G.C.WHITE, lighten(G.C.PURPLE, 0.4), lighten(G.C.PURPLE, 0.2), lighten(G.C.GOLD, 0.2)
                        },
                        fill = true
                    })
